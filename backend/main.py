...
async def recommend(request: Request):
    ...
    ollama_prompt = (
        f"Dado el siguiente criterio de búsqueda: '{query}', se han encontrado varios candidatos por un modelo de LLM pre-entrenado llamado nomic-ai/nomic-embed-text-v1. tu función es dar una breve introducción sobre los candidatos que se encontraron basado en el prompt que recibes en el query, como tal no tienes información sobre los candidatos, por lo que debes ser creativo y generar texto coherente. Recuerda que el modelo de lenguaje es muy poderoso y puede generar texto de alta calidad. Por favor, genera un texto de 3-5 oraciones sobre los candidatos encontrados, no digas nombres ni detalles específicos, ya que no sabes el nombre de los candidatos solo cosas como ¡Aquí hay una variedad de talentosos... y cosas de similar índole, no digas en que tecnologías está enfocado o en que industria trabaja, solo genera un texto general sobre los candidatos encontrados ya que no tienes acceso a esos datos específicos."
    )
